...
**Full reindex (batched + resumable).** The external index is mirrored best-effort on ingest and reconciled at boot, but an operator can force a full rebuild out-of-band — e.g. after pointing at a fresh cluster, recovering from cluster-side data loss, or flipping `OPENSEARCH_PER_TENANT_INDEX` (which changes which physical index each tenant's docs occupy). Run `pnpm --filter @workspace/api-server run reindex:search -- [--batch-size=N] [--since-tenant=<tenantId> --since-id=<findingId>]`. It reuses the same `reconcileSearchIndex` path as boot (no duplicate mirror logic): keyset-paginated per tenant by finding id, each page mirrored in one `_bulk` round-trip, progress logged per batch, and `--since-tenant` + `--since-id` (both copied from the last progress line) resume an interrupted run. Because tenants are scanned in tenant_id order rather than global id order, resume needs the tenant as well as the id — a bare id is ambiguous across tenants and is rejected; resume assumes an unchanged tenant set (run without the flags for a guaranteed-complete pass). Page size defaults to `SEARCH_REINDEX_BATCH_SIZE` (default 500, clamped 1..10000). It is a no-op under `SEARCH_PROVIDER=postgres`. Reads use the redacted safe projection only — raw PHI never reaches the searchable tier.

**Per-tenant index isolation (`OPENSEARCH_PER_TENANT_INDEX`, default off).** By default all tenants share one index (`OPENSEARCH_INDEX_PREFIX`) scoped only by the `tenant_id` term filter. Set `OPENSEARCH_PER_TENANT_INDEX=true` to give each tenant a physically separate index `${OPENSEARCH_INDEX_PREFIX}-${safeTenantSegment(tenantId)}` (sanitized + SHA-256-suffixed so distinct tenant ids never collide onto one index); the term filter stays on as defense-in-depth. Off ⇒ byte-identical to the shared-index behavior. **Operator-deferred (cluster side), like service-mesh mTLS and per-tenant KMS lifecycle:** the app addresses the correct physical index, but index-template/alias provisioning and per-index RBAC (binding each tenant index to that tenant's service identity at the cluster) are OpenSearch-administration concerns to configure outside the app.
...
The agent supervisor (M5) defaults to an **in-process queue** with `CONCURRENCY=2` per pod. Scaling api replicas multiplies effective concurrency. The HPA defaults (min=2, max=6) yield 4–12 concurrent agent jobs cluster-wide; tune `AGENT_DAILY_TOKEN_BUDGET` accordingly.

If you need durable orchestration + single-flight-per-finding semantics regardless of replica count, **externalize the queue onto Temporal** (`WORKFLOW_ENGINE=temporal`) rather than pinning replica=1 (which loses HA). See the worker-topology note below; full env in `docs/CONFIGURATION.md`.

### Temporal supervisor backend (worker topology)

`WORKFLOW_ENGINE=temporal` + `TEMPORAL_ADDRESS=<host:7233>` makes Temporal a durable, selectable backend behind the `WorkflowEngine` seam (default `inprocess` stays byte-identical). Each `finding.created` starts a workflow whose id is a stable hash of `{tenantId, findingId}`, so duplicate emits dedupe via Temporal start-workflow idempotency *plus* the existing DB CAS, and a worker crash resumes from the last *completed* activity (history replay reuses recorded results) instead of losing the job. Activity auto-retry is disabled by design (`maximumAttempts: 1`) because the activities do non-idempotent side effects (budget charge + append-only ledger) — auto-retry would duplicate audit-ledger entries; this matches the in-process engine's at-most-once-per-step semantics. Per-step idempotency keys to safely re-enable retry are a deliberate follow-up.

- **Cluster is operator-provided** (Temporal Cloud or self-hosted) — we do **not** bundle one, same posture as Kafka/OpenSearch/S3.
- **Optional SDK** — operators must `pnpm --filter @workspace/api-server add @temporalio/client @temporalio/worker @temporalio/workflow` before enabling; it is lazy-loaded, so a build without it is unaffected and the eval gate stays byte-identical.
- **Topology** — every api pod also runs a Temporal worker polling `TEMPORAL_TASK_QUEUE` (default `phi-audit-review`); N replicas scale review throughput horizontally. The per-tenant budget breaker is **per-worker** — a cluster-wide budget would need a shared store (operator-deferred).
- **Auth/TLS** — `TEMPORAL_TLS=1` for TLS to the cluster; `TEMPORAL_API_KEY` for Temporal Cloud API-key auth; `TEMPORAL_NAMESPACE` (default `default`).
- **Boot is fail-fast** — selecting `temporal` without `TEMPORAL_ADDRESS` aborts boot rather than silently falling back to in-process.
- The seam currently covers the **supervisor review workflow only**; ingest / notarizer / chain-verifier stay on their existing paths.

#### Live-cluster verification harness

`artifacts/api-server/src/lib/agents/temporal-integration.test.ts` is an opt-in integration test that drives the **real** `@temporalio/*` SDK against a **real** running Temporal server (not the mocked `TemporalRuntime` used by `workflow-engine.test.ts`). It exercises the guarantees end-to-end: lazy SDK load, bundled-workflow resolution (the worker webpack-bundles the source `temporal-workflows.ts` → `review-orchestration.ts`), workflow-id idempotency on a duplicate emit (second start swallowed as AlreadyStarted — one execution, one activity sequence), worker-crash resume from the last *completed* activity (history replay reuses the recorded triage result, only the verifier runs on the restarted worker), and **transient-failure self-healing via bounded auto-retry** (an activity rigged to throw on its first attempts — once before its side effect, once after committing its complete-ledger entry + charge — is auto-retried by the real worker per `REVIEW_ACTIVITY_OPTIONS` and dedupes to completion, asserting exactly one complete-ledger entry and one charge per step despite the retries). The activities are deterministic in-process recorders (no DB / no LLM); the happy-path case's asserted call sequence matches `InProcessWorkflowEngine` exactly, and the retry case's recorder mirrors review-steps.ts's "charge only on the dedupe-gate ledger INSERT" contract so the live exactly-once-under-retry behavior is observable without a DB.

It is gated on `TEMPORAL_INTEGRATION=1` + a reachable `TEMPORAL_ADDRESS`, so the normal `vitest run` suite and the credential-free eval gate **skip it entirely** (4 skipped, byte-identical). Run it on an adequately-resourced machine with:

```
pnpm --filter @workspace/api-server run test:temporal
```

`scripts/run-temporal-integration.sh` starts a local single-binary `temporal server start-dev` (no Docker; the CLI is found on `PATH` or in the Nix store), points the test at it, and tears the server down afterwards. The `@temporalio/*` packages live in `optionalDependencies` so a build without them — and the eval gate — is unaffected.

> Note: the underlying toolchain was confirmed in this environment — the dev server starts and serves gRPC without Docker, the native SDK loads in the api-server package, and `bundleWorkflowCode` successfully bundles the real workflow graph (1.48 MB, webpack OK). Executing a live worker for the full e2e/dedup/resume run was not reproducible in the constrained dev sandbox (the native worker + dev server OOM-kills under host memory pressure), so the test above is the harness to run it where resources allow.

**Automated in CI.** A dedicated GitHub Actions workflow — `.github/workflows/temporal-integration.yml` — now runs this harness automatically on a GitHub-hosted `ubuntu-latest` runner (which has enough memory, unlike the dev sandbox). It installs the `temporal` CLI via `temporalio/setup-temporal`, runs `pnpm install --frozen-lockfile` (which installs the optional `@temporalio/*` deps), then runs `pnpm --filter @workspace/api-server run test:temporal` — green requires all three live cases to pass against a real server. It is split out of `ci.yml` (rather than added as a per-PR job) because booting a real dev server + native worker is heavier and slower than the per-PR gates. Triggers: a **nightly** schedule (the standing gate), **manual** `workflow_dispatch`, and **PRs that touch the Temporal backend** (`temporal-engine.ts` / `temporal-workflows.ts` / `review-orchestration.ts` / the runner script / the workflow file).

**Paging on nightly failure.** A red nightly run otherwise only shows up in the Actions tab — nobody is paged, so a regression in the durable-orchestration backend can sit unnoticed. The workflow now posts a concise `high`-severity failure alert through the **same** `CHANNEL_*` notifier path + per-channel `CHANNEL_*_MIN_SEVERITY` gating as the eval gate (Slack / HMAC-signed webhook / PagerDuty Events API v2) via `artifacts/api-server/evals/temporal-notify.mjs`. It is **inert** (logs only) when no channel secret is set, so a fork or a repo without secrets pages nothing. It fires **only on the nightly `schedule`** (`if: failure() && github.event_name == 'schedule'`) — manual `workflow_dispatch` and PR re-runs never page, since a red result there is already visible to whoever triggered it. The payload is CI liveness metadata only (gate name + exit code) — **no PHI, no scores**. PagerDuty uses a distinct stable dedup key (`temporal-integration-nightly`) so a failing Temporal gate is its **own** incident, separate from the eval-gate run page (`eval-gate-nightly`) and the heartbeat page (`eval-gate-heartbeat`), and re-runs of a still-red gate fold into one incident. The notifier always exits 0, so it never changes the job result — the failed test step is what keeps the run red. Provide the `CHANNEL_*` values as repo **secrets** (URLs/keys/secret) and **vars** (`*_MIN_SEVERITY`, allowed hosts, optional PagerDuty endpoint/dedup-key); empty ⇒ inert.

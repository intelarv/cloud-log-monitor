...
Current milestone: **M26** — **deferred follow-up completion** (all default-inert, eval gate byte-identical +0.0pt): nightly eval-monitoring observability in `evals/notify.mjs` (per-suite score sparkline, K-consecutive downward-drift warning, recovery notes carrying failing duration + run-count, flapping-suppression window, DB heartbeat "run started" row for hung-run detection, per-change gate start/hung ping parity, ping-secret-set-but-job-disabled misconfig detector, PagerDuty auto-resolve on Temporal-gate recovery — all gate-summary-metadata-driven, never PHI); supervisor durability (`temporal-engine.ts` buffered-vs-dispatched metric + chain-verifier via `schedulePeriodic`, `supervisor.review_dropped` alert-classified); break-glass re-request (`routes/admin.ts` per-user/per-finding re-review rate limit + analyst-selectable grant `ttl_seconds` 60s–15m); dashboard UX (in-app break-glass note modal replacing `prompt()`, malformed-evidence-tolerant finding detail, live "reviewing…" spinner + auto-refresh, polling halted when page hidden, deep-link out-of-window ledger fetch); helm-matrix heartbeat-render assertions for AWS + Azure; committed vitest+RTL UI tests + supertest route tests + gated live-Temporal harness tests. **Full per-milestone history in `docs/MILESTONES.md` (M26 entry).** Prior milestone: **M25** — **deferred-work completion**, all default-inert (eval gate byte-identical): (1) **real remediation executor backends** behind the `RemediationExecutor` seam — `ChannelSendExecutor` (PHI-safe `remediation.notify_dispatched` via the channel router, fail-closed if no channels), `GitHubIssueExecutor` (lazy `@octokit/rest` optional dep, `REMEDIATION_GITHUB_*`, summary/rationale re-scanned before external send), `RedactionQueueExecutor` (enqueues to the new `redaction_requests` RLS table, idempotent `ON CONFLICT (tenant_id, proposal_id)`), `RoutingRemediationExecutor` (action_type→backend); default stays `DevNoopExecutor`, agent plane still never executes (HITL preserved); (2) **`local` NER coverage** — unit tests + opt-in eval leg for `NER_PROVIDER=local`, no deterministic detector change; (3) **orchestration** — `AgentBudgetStore` seam (`AGENT_BUDGET_STORE=memory|postgres`; memory default byte-identical, `postgres` = fleet-wide atomic upsert-increment on the new `tenant_budgets` RLS table; budget ops now async) + the stuck-cursor **log-source reaper migrated onto the `WorkflowEngine` seam** (in-process leader-locked setInterval byte-identical; durable cron workflow under `WORKFLOW_ENGINE=temporal`) + the two **one-shot jobs** (boot **search-index reconcile** + dev **fixture ingest replay**) migrated onto the same seam via a new `executeOneShot` capability (in-process inline byte-identical; durable awaited workflow under `WORKFLOW_ENGINE=temporal`, logged inline fallback when the engine isn't yet running — e.g. the boot reconcile that precedes engine selection). Prior milestone: **M24** — production **TOTP step-up second factor** (opt-in `STEP_UP_PROVIDER=dev|totp`, default `dev` ⇒ existing `STEP_UP_DEV_TOKEN` constant-time compare, byte-identical): RFC 6238 TOTP (`lib/totp.ts`, `node:crypto` only) behind a `StepUpVerifier` seam, per-analyst enrollment endpoints + dashboard QR setup, TOTP secret **AES-256-GCM encrypted at rest** (per-tenant KMS key via `resolveTenantSecret`, else `SESSION_SECRET`), per-step replay guard via `last_used_step`. **Full per-milestone history (M0 → M25) + a standing "Implemented vs not" survey live in `docs/MILESTONES.md`.** **Default-safe invariant:** every optional / cloud / security seam is default-inert (loads no SDK, changes no behavior unless its env/data is set), so the credential-free offline eval gate stays byte-identical.

**Docs:** full per-milestone history (M0 → M25) + a standing "Implemented vs not" survey in `docs/MILESTONES.md`; architecture + implementation decisions in `docs/ARCHITECTURE.md`; optional env reference in `docs/CONFIGURATION.md`.
...
- Required env: `DATABASE_URL`, `SESSION_SECRET`. **Every optional subsystem — full env vars, defaults, credentials, and the cloud-aware embedder-selection table — is documented in `docs/CONFIGURATION.md` ("Optional env (by subsystem)").** All seams are **default-inert**: unset ⇒ no SDK loaded, no behavior change, eval gate byte-identical. Switch families at a glance (names only; `docs/CONFIGURATION.md` has prose, defaults, and credentials per switch):
  ...
  - **Retrieval / raw-evidence** — `SEARCH_PROVIDER`·`OPENSEARCH_*`·`SEARCH_REINDEX_BATCH_SIZE`; `RAW_EVIDENCE_PROVIDER` + tiering (`RAW_EVIDENCE_TIER_*`) + write-retry (`RAW_EVIDENCE_WRITE_*`).
  ...
  - **Security / ops** — `STEP_UP_PROVIDER=dev|totp` (M24: `dev` ⇒ `STEP_UP_DEV_TOKEN`, byte-identical; `totp` ⇒ RFC 6238 TOTP via the `StepUpVerifier` seam, AES-256-GCM secret-at-rest, enrollment endpoints); `NOTARIZATION_SECRET`·`NOTARIZATION_RETIRED_KEYS` (separate trust zone); `CHANNEL_*` (Slack / HMAC webhook / PagerDuty); `A2A_REQUIRE_MTLS`·`A2A_MTLS_*` (cross-cloud mTLS + peer ABAC).
...
- Full env + embedder config reference: `docs/CONFIGURATION.md`. Per-milestone history (M0 → M25): `docs/MILESTONES.md`. Architecture + implementation decisions: `docs/ARCHITECTURE.md`.

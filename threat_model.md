...
- **A2A authorization.** A2A worker endpoints MUST verify the caller's ABAC scope; a Triage agent calling a Verifier MUST NOT be able to invoke a Remediation tool. *(Partially implemented M-A2A: Triage + Verifier are exposed as official A2A agents (`@a2a-js/sdk` — Agent Cards + JSON-RPC `message/send`) over loopback in `artifacts/api-server/src/lib/a2a/`; every A2A endpoint is shared-secret bearer authed (`a2aAuthMiddleware`, `timingSafeEqual`); only the redacted `FindingSafe` projection crosses the wire — executors parse+strip the inbound payload so `rawEvidence`/`rawEvidenceRef` cannot ride; returned rationale is re-scanned by `scanForPhi` before the ledger; per-agent scope is enforced by the Supervisor's `AgentInvoker` seam (each agent card advertises only its own skill). A default-inert **cross-cloud mTLS transport seam** (`a2a/transport.ts`, gated on `A2A_REQUIRE_MTLS`) now hardens the transport: the client lazily builds an undici dispatcher carrying `A2A_MTLS_CERT`/`A2A_MTLS_KEY`/`A2A_MTLS_CA` (optional dep, same posture as the cloud SDKs), and an outermost server middleware requires a verified client certificate — either direct TLS (`req.socket.authorized`) or an ingress-stamped verify header (configurable, default `x-ssl-client-verify=SUCCESS`) — refusing 403 before the shared-secret + caller-identity layers run, and recording the refusal on the tamper-evident ledger (`a2a.transport_rejected`, system-scoped, payload = fixed route + static reason only, never request headers/body) so the transport-auth failure is non-repudiable. **Per-claim ABAC against the mTLS peer's SAN/DN is now implemented** (`a2a/peer-identity.ts`, default-inert): when mTLS is on, the transport extracts the peer's certificate identity (SAN entries + subject CN/DN) from either direct TLS (`getPeerCertificate`) or an ingress-forwarded URL-encoded client-cert PEM / subject-DN header (parsed with `node:crypto` `X509Certificate`), and stashes it on the request. Two opt-in controls layer on top: an allow-list (`A2A_MTLS_ALLOWED_PEERS`) refuses any non-allow-listed cert identity at the transport layer (403 + ledger reason `peer_not_allowed`, or `peer_identity_unavailable` if a cert was verified but no identity could be read), and a peer↔caller binding (`A2A_MTLS_PEER_BINDINGS`, `peerId=callerSubject` pairs) ties the network identity (cert) to the application identity (the caller-identity JWT `sub`) so a confused-deputy holding a valid cert but a spoofed JWT subject is refused (403 + ledger reason `peer_subject_mismatch`). All refusals reuse the existing `a2a.transport_rejected` event (system-scoped; payload = fixed route + static reason only — the peer DN/SAN and request headers are never ledgered). Both controls are inert unless their env is set, so loopback dev and the credential-free eval gate are byte-identical. Loopback-only deployment still keeps the agent plane off the public surface when mTLS is not configured.)*

...
Current milestone: **M11 complete** (eval suite). On top of M9 (Helm chart, Postgres TF module, per-cloud TF roots, Replit deploy path, chat-agent via runtime seam): a fixture-based, score-emitting quality harness that **measures** (never changes) the detectors / redactor / agent-defense policy / chat agent. On-demand only via `pnpm --filter @workspace/api-server run eval` (separate `vitest.eval.config.ts`); six deterministic credential-free suites form a versioned baseline (`evals/baseline.json`) and `evals/gate.mjs` fails on any >5pt regression; two more suites run under `EVAL_LLM=1`. See "Eval suite" below and `artifacts/api-server/evals/README.md`. Test suite still **226 tests** passing (evals are a separate runner). Per-milestone history in `docs/MILESTONES.md`; planned M10/M12 (OpenSearch+S3, multi-tenant hardening) in the same file's "Planned (backlog)" section.

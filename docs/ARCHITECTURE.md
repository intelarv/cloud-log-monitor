...
2. ✅ **Done (M3 + M8)** — Ingest end-to-end for one log source. M3 landed the `LogRecord`/`LogSource`/`LogBus` seams + `StaticFixtureLogSource` + detector → redact → fingerprint-upsert → ledger pipeline; M8 added the real `CloudwatchLogSource` (lazy-loaded `@aws-sdk/client-cloudwatch-logs`, durable cursor in `log_source_checkpoints`, contiguous-success-prefix watermark on partial failure, lifecycle ledgered). Production broker transport is now implemented behind the same `LogBus` seam (`LOG_BUS_PROVIDER=memory|kafka|nats`): real Kafka/Redpanda (`kafkajs`) and NATS JetStream (`nats`) impls wrap a `BrokerDriver`, lazy-load their SDK on `connect()`, and are default-inert (dev/CI stay on `InMemoryLogBus`, eval gate byte-identical). At-least-once delivery (handler throw → no-ack → broker redelivers; ingest dedupes by fingerprint); wire records are JSON+Zod-revalidated on consume (poison → drop). `raw.logs` is a pre-detection transport so raw payloads ride it by design — broker TLS/SASL is the in-transit control.
3. ✅ **Done (M4)** — Dashboard against seeded findings (React + Vite + shadcn; Findings, Finding detail, Chat, Ledger, Admin pages).
4. ✅ **Done (M5 + M-Temporal)** — Supervisor + Triage/Verifier agents. In-process orchestration + in-memory queue with CAS exactly-once is the default; Temporal is now a **selectable, durable backend** behind the `WorkflowEngine` seam (`WORKFLOW_ENGINE=temporal`, lazy SDK, default-inert — in-process stays default + byte-identical). The orchestration body is shared between both engines (review steps are engine-agnostic activities); Temporal adds durable replay + workflow-id idempotency on top of the existing DB CAS. The cluster itself stays operator-provided (same posture as Kafka/OpenSearch).
...
6. ⏳ **Partial (M14)** — Context + Remediation + Notifier agents not yet built as standalone runtimes, BUT the **HITL gate on code-touching actions is shipped**: the `propose_remediation` tool (chat + future remediation agent) writes a PENDING `remediation_proposals` row + ledgers `remediation.proposed`, and the human confirm/reject endpoints (`/api/admin/remediation/proposals[/:id/confirm|/reject]`) gate execution (confirm requires step-up + is ledgered `remediation.confirmed`). Proposals only — nothing executes server-side; the actual code-touching step stays operator/out-of-band by design. The standalone Context/Notifier agents + an executing remediation worker remain unbuilt.
7. ⏳ **Partial** — pgvector + embedder factory shipped in M1.x; **importance-decay memory eviction shipped in M10.5** (`MEMORY_MAX_EMBEDDINGS_PER_TENANT` + half-life decay scorer). Cross-finding consolidation/summarization jobs remain unbuilt.
8. ✅ **Done** — Periodic chain verifier (hourly rolling-24h + weekly full) shipped in M1.x + external HMAC notarization in M2; **tiered raw-evidence storage lifecycle shipped in M10.4** (`RAW_EVIDENCE_TIER_AGE_DAYS` ages inline raw → external WORM/Object Lock store). A full hot→warm→cold *search*-tier lifecycle (OpenSearch ILM) is still an operator concern.
...
- A2A protocol for Supervisor↔Triage/Verifier (`@a2a-js/sdk`) — **M-A2A**
- Temporal as a selectable, durable supervisor backend behind the `WorkflowEngine` seam (`WORKFLOW_ENGINE=temporal`; in-process stays default + byte-identical) — **M-Temporal**

**Still deferred (live items, not yet milestone-tagged):**
- Migrating other subsystems (ingest pipeline, notarizer, chain verifier, reindex) onto Temporal — the `WorkflowEngine` seam currently covers the supervisor review workflow only.
...
- **Name detection: broader multicultural gazetteer over a runtime NER model (for the dev/default detector).** Name recall is the only Safe-Harbor class that can't be expressed as a shape regex, so it leans on a dictionary signal (`GIVEN_NAMES`/`SURNAMES` + casing heuristic + title/context single-name passes in `redact.ts`). A real NER engine (Presidio / spaCy / clinical-BERT, §11) is the documented **production** path behind the same detector interface, but it needs a heavyweight dependency + downloaded model weights, which is incompatible with the deterministic, credential-free, offline `eval-gate` that runs on every change. So the dev/default detector instead extends the curated lists with common given names and surnames from South Asian, East Asian (Chinese/Japanese/Korean), Arabic/Persian/Turkish, African, Slavic, Vietnamese, and Greek naming traditions, and adds a patient-context single-name pass (`NAME_CONTEXT` keyword + dictionary name → flag, e.g. "patient Ngozi") alongside the existing title pass. The "dictionary-membership required" rule on the adjacent-pair / title / context passes keeps precision at 1.0 on the benign operational controls (the casing heuristic still needs ≥2 capitalized tokens). Tokens that collide with ordinary English words (Tang, Dang, Sun, Park, An, Li, Le, Song, Moon, Long) are deliberately kept out of the gazetteer. The **context-anchored** half of that collision case is now closed deterministically (M13.3 partial): a separate `COLLISION_SURNAMES` set is consulted only when a person-context keyword precedes the token, the token is Capitalized, and the separator is not a sentence-ending period ("patient Sun" / "member Li" / "enrollee: Park" recall; "the sun set", "members park …", "patient. Sun exposure …", "patient parking …" stay silent). "An" is excluded even here as too-common. The remaining **un-anchored** collision case ("Park reviewed the chart") — and free-form addresses — stay the known limitation the production NER path closes. Measured by the `detector-phi` eval over the diverse fixtures in `evals/fixtures/phi.ts`.
- **The Stage-1 → NER recall gap is quantified by an OPT-IN measurement, kept out of the gated baseline.** The un-anchored / word-collision names Stage-1 cannot precision-safely match live in their OWN fixture export (`NER_PHI_FIXTURES` in `evals/fixtures/phi.ts`), never in the gated `PHI_FIXTURES` — folding them in would manufacture unexplained false negatives against a *deterministic* detector and regress the gated `detector-phi` recall. The `detector-ner` eval (`evals/detector-ner.eval.ts`) measures three numbers over those fixtures: `stage1_recall` (Stage-1 alone, the documented gap), `ner_recall` (Stage-1 ∪ optional NER spans, via `scanForPhiWithNer`), and `benign_fp` (false positives the merged pipeline adds on the benign operational controls, reusing `BENIGN_FIXTURES`). The recorded `score` is `ner_recall`; `recall_gain = ner_recall − stage1_recall` is the headline. It is **opt-in behind `EVAL_NER=1`** (mirroring the `EVAL_LLM` gating of the live suites): unset ⇒ the suite is skipped, writes no result file, and `eval-gate` stays byte-identical to the credential-free baseline. The offline stand-in is a deterministic, credential-free fake that matches a fixed name lexicon at a case-sensitive word boundary (no cloud SDK, no network); a real cloud provider (`aws-comprehend` / `gcp-dlp` / `azure-language`, `NER_PROVIDER`) plugs into the same merge path. Current reading: `stage1_recall = 0`, `ner_recall = 1.0`, `benign_fp = 0` — the seam recovers every labeled name with no precision cost on the benign controls.
...
- **Raw-evidence tiering ages already-stored inline raw PHI out of the hot tier; it is NOT the ingest write path.** `RAW_EVIDENCE_PROVIDER` only governs where *new* ingests write raw evidence — every finding ingested before an operator switches from the inline `database` store to an external WORM store keeps its raw PHI in the hot `findings.raw_evidence` jsonb column forever. `raw-evidence-tiering.ts` closes that lifecycle gap: a leader-locked periodic job (its own advisory-lock key, distinct from the chain-verifier/ledger locks) scans each tenant (cross-tenant `SELECT DISTINCT tenant_id` then per-tenant `withTenant` so RLS holds on every read+write, mirroring `backfillEmbeddings`) for findings whose `last_seen_at` is older than `RAW_EVIDENCE_TIER_AGE_DAYS`, writes the `{first,latest}` snapshots to the external store as immutable objects, and only then nulls the inline column + seats `raw_evidence_ref`. **Get-after-put before destroy:** the inline copy is frequently the *sole* copy of that PHI, so the job re-reads the WORM objects and confirms they're readable before the `UPDATE … raw_evidence = NULL` (guarded by `raw_evidence IS NOT NULL` so a concurrent re-ingest can't be clobbered — 0 rows ⇒ no ledger, retry next cadence). **Default-INERT in two independent layers:** scheduled only when `RAW_EVIDENCE_TIER_AGE_DAYS` is set AND an external store is active — the inline store has no WORM tier to age into, so dev boot + the offline eval gate are byte-identical (a boot WARN fires if age is set but the store is still inline). Success ledgers `raw_evidence.tiered` (NOT_ALERTABLE — routine, *reduces* exposure); any put/get/verify failure preserves the inline copy, ledgers `raw_evidence.tier_failed` (warning — unlike the ingest write-loss event nothing is lost, but degraded hot→WORM migration usually signals the same object-store outage that's also failing ingest writes), and retries next cadence. Break-glass reads are unaffected (they already resolve `raw_evidence_ref` first with inline fallback). Ledger payloads + logs carry finding id + provider name only — never raw PHI, never object URIs (those live in `raw_evidence_ref`, out of `findingSafeColumns`).
- **Vector memory is a bounded, evictable cache — not the audit record — so a deterministic importance policy prunes it without LLM cost or touching the ledger.** `finding_embeddings` is a DERIVED table (one pgvector row per finding, rebuilt by `backfillEmbeddings` from the redacted safe projection) that powers only the *vector* leg of hybrid retrieval. The findings audit record is append-only and RLS-locked; the embedding cache, by contrast, can be pruned freely — deleting a row only removes a finding from the *vector* retriever, while the lexical/BM25 leg (Postgres FTS / OpenSearch) still retrieves it and a break-glass reader still resolves its raw evidence. So `memory-eviction.ts` bounds that cache without ever degrading the audit trail or recall for anything an analyst is likely to need. It prunes two ways, both deterministic (no LLM, mirroring the precision-first detector discipline): (1) **consolidation (group-dedup)** — among OLD (older than the half-life) or RESOLVED/false-positive findings sharing `(classification, subclass, source)`, keep only the single highest-importance representative's embedding and evict the duplicate noise; recent OPEN findings are never consolidated (they may be distinct live incidents); (2) **count cap** — after consolidation, keep at most `MEMORY_MAX_EMBEDDINGS_PER_TENANT` per tenant (top-N by importance). **Hard floor:** a critical-severity still-open finding is NEVER evicted by either pass regardless of age/cap. `computeImportance` is a pure score: severity weight × `2^(-ageDays/halfLife)` recency decay on `last_seen_at` + `log2(1+occurrence_count)` bonus − a closed-status penalty (future-dated `last_seen_at` clamped to age 0 so a clock skew can't inflate). **KEY INVARIANT — one policy, shared by create AND evict:** the same `selectEvictions` eligibility gates both `backfillEmbeddings` (it refrains from CREATING an embedding the policy would immediately evict) and the eviction job (it REMOVES ineligible embeddings). Without the shared gate, the next boot backfill would recreate whatever eviction just removed (recreate thrash); with it, the two converge on one steady state. Operationally it mirrors the raw-evidence tiering job: **default-INERT** (scheduled only when `MEMORY_MAX_EMBEDDINGS_PER_TENANT` is set — unset ⇒ backfill byte-identical, dev boot + offline eval gate unchanged), **leader-locked** (its own advisory key, distinct from the ledger/chain-verifier/tiering locks; `setInterval().unref()`), cross-tenant scan (`SELECT DISTINCT tenant_id` then per-tenant `withTenant` so RLS holds on every read+delete), and read+compute+delete share ONE per-tenant transaction so the decision is consistent with the rows deleted. Success ledgers `memory.evicted` (NOT_ALERTABLE — routine cache maintenance); a per-tenant failure ledgers `memory.evict_failed` (warning — embeddings left intact, retried next cadence). Ledger payloads + logs carry counts + policy parameters only — never PHI, snippets, or finding ids (the embedding `content` is redacted-only to begin with, and eviction never reads it). No schema change (`finding_embeddings`, `last_seen_at`, `occurrence_count` all pre-exist).

- **Temporal is a selectable, durable supervisor backend behind the `WorkflowEngine` seam — the in-process engine stays the default and byte-identical.** The supervisor's Triage→Verifier review on every `finding.created` historically ran on an in-memory queue (CONCURRENCY=2 + CAS `pending→in_progress` for exactly-once). That is demo-scale: an API restart mid-review loses in-flight + queued jobs, retries are best-effort, and there is no durable orchestration history. Rather than make Temporal a hard dependency, it slots in behind a new `WorkflowEngine` seam (`submitReview` / `start` / `stop`) selected by `WORKFLOW_ENGINE` — the exact default-inert pattern of `LOG_BUS_PROVIDER` / `RAW_EVIDENCE_PROVIDER` / `LLM_PROVIDER`. **Key move: the per-finding review body is factored into engine-agnostic, individually-retryable activities** (acquire-via-CAS, triage call + PHI-scan + ledger, inter-step budget recheck, verifier call + PHI-scan + ledger, persist verdicts, failure-path persist+ledger) exposed through a pure `ReviewActivities` interface + a pure `runReviewOrchestration` driver. The `InProcessWorkflowEngine` calls those activities directly (so its ledger events, budget breaker, CAS, concurrency, and PHI-before-ledger ordering are *identical* to the old `reviewOne` — verified by `supervisor.test.ts`); the `TemporalWorkflowEngine` runs the SAME driver as a deterministic Temporal workflow with the activities registered on a worker (orchestration-only workflow, all I/O in activities, so a worker crash resumes from the last *completed* activity — replay reuses recorded results, it does not re-run them). **Activity auto-retry is enabled with bounded backoff (`maximumAttempts: 5`, exponential `1s→30s`, `REVIEW_ACTIVITY_OPTIONS` in the pure orchestration module)** so transient step failures (LLM timeout, A2A blip, DB hiccup) self-heal instead of dropping the finding into `failed`. This is safe because each side-effecting step is now **per-step idempotent**: the activity derives a stable key from `workflowIdFor(tenantId, findingId)` + the step name and passes it as `appendLedger`'s `idempotencyKey`, which dedupes on the `ledger_idempotency_key_uniq` unique index inside the advisory-locked writer txn (the key is NOT part of `computeLedgerHash`, so the chain stays byte-identical and pre-existing NULL-keyed rows coexist). A retried step that already wrote its `agent.*_complete` entry short-circuits — it recovers the recorded verdict from that entry and returns without re-calling the LLM, re-charging the budget, or writing a duplicate audit row (the post-commit alert/supervisor/channel fan-out is skipped on a dedupe hit too). The per-tenant budget charge is ordered AFTER the dedupe-gate ledger write so a crash-then-retry charges exactly once (the only residual is a rare single-step undercount if a crash lands in the tiny window between the ledger write and the charge — acceptable for the per-process best-effort budget). Exactly-once-per-finding still additionally holds via the CAS + workflow-id idempotency. The in-process engine never retries, so its dedupe check always misses and its behavior — plus the offline eval gate — stays byte-identical. Workflow id = `review-` + sha256(length-prefixed `{tenantId, findingId}`).slice(0,32), so a duplicate emit dedupes via Temporal start-workflow idempotency *in addition to* the DB CAS (length-prefix avoids a `:`-delimiter collision when an id itself contains a colon). The Temporal SDK (`@temporalio/{client,worker,workflow}`) is an **optional dependency** lazy-loaded via a variable-aliased dynamic import (esbuild can't see it — same hidden-import trick as the cloud embedders/LLM SDKs), the workflow module is bundled as a separate esbuild entry with `@temporalio/*` left external (Temporal needs to load it in its own deterministic v8 isolate, so it's excluded from the leaf-package tsconfig + emitted standalone), and **boot fails fast** if `WORKFLOW_ENGINE=temporal` is set without `TEMPORAL_ADDRESS` (a misconfigured opt-in must never silently fall back to in-process). **Dropped/undispatched reviews surface on the ledger, not just in logs:** the engine buffers reviews during its async startup window (bounded at `MAX_PENDING_SUBMITS=10_000`) and dispatch is fire-and-forget with no retry, so a pre-start buffer overflow or a non-idempotency `startWorkflow` failure would otherwise silently mean a finding never got its Triage→Verifier verdict. Both now ledger `supervisor.review_dropped` (tenant-scoped; payload = finding id + a static `reason` [`prestart_buffer_overflow` / `start_workflow_failed`] + buffer cap / error name only — no PHI), wired into `ALERT_RULES` at `high` (same reasoning as `ingest.dead_lettered`: a missed review an operator must replay proactively). An `AlreadyStarted` failure is the idempotency guarantee, not a drop, so it is swallowed without alerting. The event is inert under the default in-process engine (which never buffers or starts workflows), so the offline eval gate stays byte-identical. We do NOT stand up a Temporal cluster in Replit dev — the adapter is unit-tested against a mocked client/worker (`workflow-engine.test.ts`), same posture as Kafka/OpenSearch/S3. Per-worker budget breaker (a cluster-wide budget would need a shared store) + migrating other subsystems (ingest, notarizer, chain verifier) onto Temporal stay operator/future-deferred.

...
Current milestone: **M13 deterministic slices complete** (detector coverage expansion). On top of M11 (eval suite — fixture-based, score-emitting quality harness that **measures** the detectors / redactor / agent-defense policy / chat agent; on-demand via `pnpm --filter @workspace/api-server run eval`, six deterministic credential-free suites baselined in `evals/baseline.json`, `evals/gate.mjs` fails on any >5pt regression, two more suites under `EVAL_LLM=1`): M13 closes the recorded PHI/secret detection gaps in `scanForPhi` (`lib/redact.ts`) under the same precision discipline. New detectors — `dob_date` (M13.1, birth-context-anchored non-slash dates), `ipv6`/`license_plate`/`passport`/`dea` (M13.2, DEA checksum-gated), eight prefix-/context-anchored secret classes + `high_entropy_secret` (M13.4/M13.5) — each ships only when it fires on the labeled fixture WITHOUT tripping the benign operational-log precision controls. detector-phi and detector-secrets re-baselined at **1.0** with zero benign false positives. **M13.3 (word-collision names / NER) is DEFERRED** — needs a production NER model incompatible with the credential-free offline eval gate. **M10.1 complete** (pluggable lexical search: `SEARCH_PROVIDER=postgres|opensearch`, OpenSearch behind a `LexicalSearchProvider` seam mirroring the embedder factory — only the BM25 leg moves; pgvector + RRF unchanged; redacted-only mirror, tenant-filtered, boot-reconciled). Test suite **393 tests** passing (evals are a separate runner). Per-milestone history in `docs/MILESTONES.md`; planned M10.2/M10.3/M12 (S3 WORM raw-PHI, multi-tenant hardening) + the deferred M13.3 NER slice in the same file's "Planned (backlog)" section.
